...
public class ArrayUtil {
	...
	public static String joinSqlIn(List<?> arr) {
		int len = arr.size();
		String separator = ",";
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < len; i++) {
	...
	/*
	 * // 排序 public static int[] sortAcs(int[] array) { if (array == null) { return null; } int[] srcdata = (int[]) array.clone(); int size = array.length; for (int i = 0; i < size; i++) for (int j = i; j < size;
	 * j++) { if (srcdata[i] > srcdata[j]) { swap(srcdata, i, j); } } return srcdata; }
	 * 
	 * public static String[] sortAcs(String[] array) { int array2[] = new int[array.length]; for (int i = 0; i < array.length; i++) { array2[i] = Integer.valueOf(array[i]); } array2 = ArrayUtil.sortAcs(array2); for
	 * (int i = 0; i < array.length; i++) { array[i] = String.valueOf(array2[i]); } return array; }
	 * 
	 * public static void swap(int[] srcdata, int src, int dest) { int temp = srcdata[src]; srcdata[src] = srcdata[dest]; srcdata[dest] = temp; }
	 */
